defmodule OresMiddleware.Plug do
  ...
  def init(opts) do
    config =
      Keyword.get_lazy(opts, :config, fn ->
        OresMiddleware.default_config(Keyword.get(opts, :service_name, "ores-service"))
      end)
      ...
      {:ok, conn, context, started_ms, idempotency_key, previous_context, previous_metadata} ->
        ...
        response =
          case result do
            ...
            {:error, :deadline_exceeded} ->
              problem(conn, 504, "deadline_exceeded", "request deadline exceeded")

            {:error, _} ->
              problem(conn, 500, "internal_error", "request handler failed")
          end
  ...
  defp begin_request(conn, stack) do
    ...
    with :ok <- check_payload(conn, config),
      ...
      {:ok, assign(conn, :ores_middleware_context, context), context,
  ...
  defp check_accept(conn, config) do
    ...
    if is_nil(accept) or accept == "*/*" or
         Enum.any?(config.settings.contentRepresentations, &String.contains?(accept, &1)),
       do: :ok,
       else: {:error, 406, "not_acceptable", "no supported representation was requested"}
  end
  ...
  defp build_context(conn, config) do
    ...
    request_id =
      valid_token(first_req_header(conn, config.settings.requestIdHeader)) || random_id()
    ...
    %{
      ...
      tenant_id: nil,
      user_id: nil,
      locale: first_req_header(conn, "accept-language"),
      started_at_unix_ms: started,
      deadline_unix_ms: started + config.settings.timeoutMs,
      baggage: %{}
    }
  end

  defp authorize_ip(conn, context, stack),
    do:
      if(stack.hooks.authorize_ip.(conn, context),
        do: :ok,
        else: {:error, 403, "ip_policy_denied", "request source is not permitted"}
      )

  defp authenticate(conn, context, stack) do
    bypass =
      stack.config.settings.testAuthBypass.enabled and
        first_req_header(conn, stack.config.settings.testAuthBypass.headerName) == "true"

    result =
      cond do
        bypass and stack.config.environment in [:test, :staging] ->
          stack.hooks.resolve_test_identity.(conn, context)

        bypass ->
          {:error, :production_forbidden}

        true ->
          stack.hooks.authenticate.(conn, context)
      end
      ...
      {:ok, decision} ->
        context = %{
          ...
          | user_id: decision[:user_id],
            ...
            baggage: Map.new(decision[:baggage] || %{})
        }

        if stack.config.integrations.sharedAuth.mode != :disabled and is_nil(context.user_id),
          do: {:error, 401, "authentication_required", "shared-auth did not establish a user"},
          else: {:ok, context}

      _ ->
        {:error, 401, "authentication_failed", "authentication failed"}
    end
  end
  ...
  defp inject_fault(config) do
    ...
    cond do
      policy.enabled and :rand.uniform() < policy.dropRate ->
        {:error, 503, "fault_drop", "injected transport drop"}

      policy.enabled and :rand.uniform() < policy.errorRate ->
        {:error, 500, "fault_error", "injected middleware error"}
    ...
    if policy.enabled and conn.method in policy.requiredMethods and is_binary(key) and key != "" do
      ...
      case stack.hooks.idempotency_get.(cache_key) do
        {:ok, %{status: status, headers: headers, body: body}} ->
          {:cached, conn |> merge_resp_headers(headers) |> send_resp(status, body) |> halt()}

        _ ->
          {:continue, conn, cache_key}
      end
    else
      {:continue, conn, nil}
    end
  ...
  defp finish_request(conn, stack, context, started_ms, idempotency_key) do
    ...
    conn =
      conn
      |> attach_correlation(stack.config, context)
      |> attach_security(stack.config)
      |> attach_etag()
      |> maybe_compress(stack.config)

    stack.hooks.schema_capture.(conn)

    conn =
      case stack.hooks.sync_observe.(context, conn, duration) do
        :ok -> conn
    ...
    if idempotency_key && conn.status in 200..299 do
      stack.hooks.idempotency_put.(
        idempotency_key,
        %{status: conn.status, headers: conn.resp_headers, body: conn.resp_body || ""},
        stack.config.settings.idempotency.ttlSeconds
      )
    end
  ...
  defp attach_security(conn, config) do
    ...
    |> put_resp_header(
      ...
      "max-age=#{policy.hstsMaxAgeSeconds}; includeSubDomains"
    )
  ...
  defp attach_etag(%{method: "GET", status: 200, resp_body: body} = conn) when is_binary(body) do
    ...
    if first_req_header(conn, "if-none-match") == etag,
      do: %{put_resp_header(conn, "etag", etag) | status: 304, resp_body: ""},
      else: put_resp_header(conn, "etag", etag)
  end
  ...
  defp maybe_compress(%{resp_body: body} = conn, config) when is_binary(body) do
    ...
    if policy.enabled and byte_size(body) >= policy.minimumBytes and
         String.contains?(first_req_header(conn, "accept-encoding") || "", "gzip") and
         is_nil(get_resp_header(conn, "content-encoding") |> List.first()) do
      conn
      |> put_resp_header("content-encoding", "gzip")
      |> delete_resp_header("content-length")
      |> update_resp_header("vary", "accept-encoding", &(&1 <> ", accept-encoding"))
      |> Map.put(:resp_body, :zlib.gzip(body))
    else
      conn
    end
  end
  ...
  defp first_req_header(conn, name),
    do: conn |> get_req_header(String.downcase(name)) |> List.first()

  defp valid_token(value) when is_binary(value),
    do: if(byte_size(value) <= 128 and Regex.match?(~r/^[A-Za-z0-9._-]+$/, value), do: value)
