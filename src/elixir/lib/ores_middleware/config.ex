defmodule OresMiddleware.Config do
  ...
  def validate(config) when is_map(config) do
    ...
    |> add(
      not positive_integer?(get_in(config, [:settings, :timeoutMs])),
      "/settings/timeoutMs",
      "range",
      "timeout must be positive"
    )
    |> add(
      not positive_integer?(get_in(config, [:settings, :maxBodyBytes])),
      "/settings/maxBodyBytes",
      "range",
      "body limit must be positive"
    )
    |> add(
      invalid_rate_limit?(config),
      "/settings/rateLimit",
      "invalid_rate_limit",
      "enabled token bucket requires positive capacity and refill"
    )
    |> add(
      invalid_fault_rates?(config),
    ...
    |> add(
      config[:environment] == :production and
        get_in(config, [:settings, :faultInjection, :enabled]) == true,
    ...
    |> add(
      config[:environment] == :production and
        get_in(config, [:settings, :testAuthBypass, :enabled]) == true,
      "/settings/testAuthBypass/enabled",
      "production_forbidden",
      "test auth bypass is forbidden in production"
    )
    |> add(
      get_in(config, [:integrations, :sharedAuth, :failOpen]) == true,
      "/integrations/sharedAuth/failOpen",
      "auth_fail_open",
      "shared-auth must fail closed"
    )
    |> add(
      get_in(config, [:settings, :tls, :mode]) == :trusted_proxy and
        get_in(config, [:settings, :tls, :trustedProxyCidrs]) == [],
      "/settings/tls/trustedProxyCidrs",
      "trusted_proxy_required",
      "trusted-proxy mode requires explicit CIDRs"
    )
  ...
  defp add(issues, true, path, code, message),
    do: [%{path: path, code: code, message: message} | issues]
  ...
  defp invalid_rate_limit?(config) do
    ...
    policy[:enabled] == true and
      (not positive_integer?(policy[:capacity]) or not is_number(policy[:refillPerSecond]) or
  ...
  defp invalid_fault_rates?(config) do
    ...
    Enum.any?([policy[:errorRate], policy[:dropRate]], fn value ->
