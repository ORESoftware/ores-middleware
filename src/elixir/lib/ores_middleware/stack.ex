defmodule OresMiddleware.Stack do
  ...
  def new!(config, hooks \\ %{}, logger \\ nil) do
    case new(config, hooks, logger) do
      ...
      {:error, issues} ->
        raise ArgumentError, "invalid middleware configuration: #{inspect(issues)}"
    end
  end
  ...
  def default_hooks do
    %{
      ...
      rate_limit: fn key, capacity, refill ->
        OresMiddleware.TokenBucket.allow(key, capacity, refill)
      end,
      idempotency_get: fn key -> OresMiddleware.IdempotencyStore.get(key) end,
      idempotency_put: fn key, response, ttl ->
        OresMiddleware.IdempotencyStore.put(key, response, ttl)
      end,
      trusted_proxy?: fn conn, cidrs -> OresMiddleware.IP.in_cidrs?(conn.remote_ip, cidrs) end,
      telemetry_started: fn context, conn ->
      ...
      telemetry_finished: fn context, conn, duration_ms ->
