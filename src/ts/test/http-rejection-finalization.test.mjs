...
import { createMiddleware, currentContext, defaultConfig } from "../dist/index.js";
import { expressMiddleware, nestjsMiddleware } from "../dist/adapters.js";
...
function portableRequest(requestId, slot, init = {}) {
  ...
  return new Request(`http://example.test/rejections/${slot}`, { ...init, headers });
}
...
test("valid tracer-owned response metadata is preserved and Vary is canonicalized", async () => {
...
test("parallel early rejections cannot bleed correlation context", async () => {
  ...
  for (const [slot, response] of responses.entries()) assertFinalized(response, `r${slot}`, 403);
  assert.equal(currentContext(), undefined);
  assert.equal(getLogContext(), undefined);
});
