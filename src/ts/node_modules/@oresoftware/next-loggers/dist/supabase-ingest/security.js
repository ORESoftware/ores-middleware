...
export function assertClientCredential(value, label) {
...
export function assertClientToken(token, allowUnauthenticated) {
    if (!token) {
        if (!allowUnauthenticated) {
            throw new Error('Supabase telemetry ingestion requires a user access token; ' +
...
export function functionUrl(options) {
    let parsed;
    try {
    ...
    if (parsed.protocol !== 'https:') {
        const allowedLocalHttp = parsed.protocol === 'http:' &&
            options.allowInsecureLocalhost === true &&
            isLocalhost(parsed.hostname);
        if (!allowedLocalHttp) {
            throw new TypeError(`Supabase ingest URL must use HTTPS; got ${parsed.protocol} for ${parsed.hostname}`);
        }
    }
