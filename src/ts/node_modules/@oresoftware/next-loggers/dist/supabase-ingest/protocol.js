...
export function timerUnref(timer) {
...
export function parseRetryAfter(value, maximumMillis, nowMillis = Date.now()) {
    if (!value) {
        return undefined;
    }
...
export async function resolveToken(value) {
...
function acknowledgementInteger(value, field) {
    if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
        throw new SupabaseIngestProtocolError(`Supabase telemetry commit acknowledgement has invalid ${field} count`);
    }
    return value;
}
export async function parseAcknowledgement(response, batch, required) {
    ...
    try {
        ...
        throw new SupabaseIngestProtocolError(`Supabase telemetry ingest did not return JSON commit acknowledgement for batch ${batch.batchId}`);
    }
    if (!data || typeof data !== 'object' || Array.isArray(data)) {
        throw new SupabaseIngestProtocolError('Supabase telemetry commit acknowledgement must be an object');
    }
    const value = data;
    if (value.schema !== 'next-loggers/ingest-ack/v1') {
        throw new SupabaseIngestProtocolError('Supabase telemetry commit acknowledgement has an invalid schema');
    }
    if (value.batchId !== batch.batchId) {
        throw new SupabaseIngestProtocolError(`Supabase telemetry commit acknowledgement batchId mismatch: expected ${batch.batchId}`);
    }
    ...
    if (requested !== requestedCount || accepted + duplicates !== requestedCount) {
        throw new SupabaseIngestProtocolError('Supabase telemetry commit acknowledgement does not account for the complete batch');
    }
    const committedAt = value.committedAt;
    if (typeof committedAt !== 'string' || Number.isNaN(Date.parse(committedAt))) {
        throw new SupabaseIngestProtocolError('Supabase telemetry commit acknowledgement has invalid committedAt');
    }
    return {
        schema: 'next-loggers/ingest-ack/v1',
        batchId: batch.batchId,
