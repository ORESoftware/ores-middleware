...
import { byteLength, generateBatchId, isRetryableError, isRetryableStatus, parseAcknowledgement, parseRetryAfter, resolveToken, timerUnref, } from './protocol.js';
export class SupabaseIngestTransport {
    name = 'supabase-ingest';
    options;
    endpoint;
    ...
    createBatch(exit) {
        ...
        const maximumBytes = Math.max(1_024, exit
    ...
    scheduleRetry(error) {
        ...
        this.retryTimer = setTimeout(() => {
            ...
            void this.flush().catch((nextError) => {
                ...
                if (isRetryableError(nextError))
                    this.scheduleRetry(nextError);
            });
        }, delay);
    ...
    async post(batch, keepalive = false) {
        ...
        try {
            ...
            if (!response.ok) {
                const retryAfterMillis = parseRetryAfter(response.headers.get('retry-after'), Math.max(0, this.options.maxRetryAfterMillis ?? 60_000), this.now().getTime());
            ...
            const parsed = await parseAcknowledgement(response, batch, this.options.requireCommitAcknowledgement !== false);
...
export function createSupabaseIngestTransport(options) {
