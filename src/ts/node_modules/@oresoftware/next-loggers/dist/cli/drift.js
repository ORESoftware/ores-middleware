...
import { COMMANDS, GLOBAL_FLAGS, LIBRARY_CONTRACT_FLAGS } from './spec.js';
function readFlagTable(scope, key, table) {
    ...
    const flag = { scope, key, env, aliases, type, help };
...
export function collectDeclared(document) {
    const flags = [];
    const commands = [];
    const walkFlags = (scope, node) => {
        if (node === undefined) {
            return;
        }
...
function specFlags() {
    const out = GLOBAL_FLAGS.map((flag) => toDeclared('global', flag));
...
function toDeclared(scope, flag) {
    const declared = {
        scope,
        key: flag.key,
        env: flag.env,
...
export function compare(document) {
    ...
    for (const [name, command] of compiledCommands) {
        ...
        if (other && other.help !== command.summary) {
            mismatchedCommands.push(`${name}: help ${JSON.stringify(other.help)} vs ${JSON.stringify(command.summary)}`);
        }
    }
    ...
    for (const flag of [...compiled, ...declared.flags]) {
        if (contractEnvs.has(flag.env) && flag.default !== undefined) {
            policyViolations.push(`${identity(flag)} declares default="${flag.default}" but ${flag.env} is read by envToLoggerOptions()`);
        }
    }
...
export function compareSource(tomlSource) {
