...
import { PACKAGE_RELEASES, RELEASE_TARGET_NAMES, ZED_TARGET_NAMES, releaseTag, } from '../package-catalog.js';
import { isStrictSemVer } from '../semver.js';
import { asTable, parseToml } from '../toml.js';
function expectString(errors, actual, expected, label) {
    if (actual !== expected) {
        errors.push(`${label}: expected "${expected}", found ${JSON.stringify(actual)}`);
    }
}
function staticCatalogErrors() {
    const errors = [];
    ...
    for (const release of PACKAGE_RELEASES) {
        ...
        if (placeholders !== 1) {
            errors.push(`${release.target} tag format must contain exactly one {version} placeholder`);
        }
...
async function checkCatalog(packageRoot, releaseVersion, packageVersion) {
    const errors = staticCatalogErrors();
    if (releaseVersion !== packageVersion) {
        errors.push(`requested release version ${releaseVersion} does not match package.json ${packageVersion}`);
    }
    let source;
    try {
    ...
    try {
        ...
        const expectedZedTargets = new Set(ZED_TARGET_NAMES);
        ...
        for (const release of PACKAGE_RELEASES) {
            ...
            if (release.manifestNative) {
                ...
                errors.push(`${targetLabel}.native should remain absent; ${release.language} is published to ${release.registry} by the tag workflow`);
            }
        }
    }
    catch (error) {
...
function filterReleases(targetFilters, registryFilters) {
    const errors = [];
    const knownTargets = new Set(RELEASE_TARGET_NAMES);
    const knownRegistries = new Set(PACKAGE_RELEASES.map((release) => release.registry));
    for (const target of targetFilters) {
        if (!knownTargets.has(target)) {
            errors.push(`unknown package target "${target}"; expected one of ${RELEASE_TARGET_NAMES.join(', ')}`);
        }
    }
    for (const registry of registryFilters) {
        if (!knownRegistries.has(registry)) {
            errors.push(`unknown registry "${registry}"; expected one of ${[...knownRegistries].sort().join(', ')}`);
        }
    }
    const targetSet = new Set(targetFilters);
    const registrySet = new Set(registryFilters);
    const releases = PACKAGE_RELEASES.filter((release) => (targetSet.size === 0 || targetSet.has(release.target)) &&
...
function renderRows(releases, version) {
    ...
    return [
        ...
        ...rows.map((row) => `${row.target.padEnd(widths.target)}  ${row.registry.padEnd(widths.registry)}  ` +
            `${row.packageName.padEnd(widths.packageName)}  ${row.tag.padEnd(widths.tag)}  ${row.environment}`),
    ];
}
export async function runPackages(ctx) {
    let packageVersion;
    try {
    ...
    if (!isStrictSemVer(releaseVersion)) {
        ctx.printErr(`next-loggers packages: --release-version must be full semantic versioning, got "${releaseVersion}"`);
        return { exitCode: 2 };
    }
    const selection = filterReleases(normalizeFilters(ctx.list('target')), normalizeFilters(ctx.list('registry')));
