 ...
 * `worker` there.
 */
function resolveNode(node, conditions) {
    if (node === null) {
        return undefined;
    }
...
export async function runResolve(ctx) {
    let manifest;
    try {
        manifest = JSON.parse(await readFile(join(ctx.packageRoot, 'package.json'), 'utf8'));
    }
    catch (error) {
    ...
    if (explicit.length > 0) {
        ...
        if (!known) {
            ctx.printErr(`next-loggers resolve: unknown runtime "${runtime}"; expected one of ${Object.keys(RUNTIME_CONDITIONS).join(', ')}`);
            return { exitCode: 2 };
        }
        conditions = known;
    }
    else {
        conditions = RUNTIME_CONDITIONS.node;
    }
