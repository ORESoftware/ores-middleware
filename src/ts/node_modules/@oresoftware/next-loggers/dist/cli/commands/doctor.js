...
export async function runDoctor(ctx) {
    const checks = [];
    const add = (check) => void checks.push(check);
    const runtime = typeof globalThis.Bun !== 'undefined'
        ? 'bun'
        : typeof globalThis.Deno !== 'undefined'
            ? 'deno'
            : 'node';
    add({ name: 'runtime', status: 'PASS', detail: runtime });
    const importFrom = async (relative) => (await import(pathToFileURL(join(ctx.packageRoot, relative)).href));
    // ── Async context ───────────────────────────────────────────────────────
    try {
        const context = await importFrom('dist/context.js');
        const tracked = context.isAsyncContextTracked();
        add(tracked
            ? {
                name: 'async context',
                status: 'PASS',
            ...
            : {
                ...
                remedy: 'On Cloudflare Workers set compatibility_flags = ["nodejs_als"]; in browsers attach a store via logger.setALS().',
            });
    }
    catch (error) {
        add({
    ...
    try {
        ...
        const load = config.loadNextLoggerConfig;
    ...
    const capability = (name, present, remedy) => {
        add({
            name,
    ...
    capability('fetch', typeof globalThis.fetch === 'function', 'HttpTransport and error tracking need fetch; pass options.fetch on older runtimes.');
    capability('WebSocket', typeof globalThis.WebSocket === 'function', 'Supabase Realtime and the browser stream need a WebSocket; pass a factory otherwise.');
