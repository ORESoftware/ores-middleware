 ...
 */
function acceptsSeparatedValue(type, token) {
    if (token === undefined) {
        return false;
    }
 ...
 */
export function applyDefaults(flags, parsed, env) {
    const out = { ...parsed };
...
export function readFlag(flag, parsed, env) {
    return parsed[flag.env] ?? env[flag.env];
}
export function readBool(flag, parsed, env) {
    const raw = readFlag(flag, parsed, env);
    return raw !== undefined && TRUE_WORDS.has(raw.toLowerCase());
}
export function readArray(flag, parsed, env) {
    const raw = readFlag(flag, parsed, env);
    if (!raw) {
        return [];
    }
