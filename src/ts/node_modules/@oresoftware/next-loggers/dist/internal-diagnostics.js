 ...
 */
export const INTERNAL_DIAGNOSTIC_SCHEMA = 'ores.otel.log/internal-diagnostic/v1';
export const INTERNAL_DIAGNOSTIC_BATCH_SCHEMA = 'ores.otel.log/internal-diagnostic-batch/v1';
export const INTERNAL_DIAGNOSTIC_UPLOAD_GRANT_SCHEMA = 'ores.otel.log/internal-diagnostic-upload-grant/v1';
export const INTERNAL_DIAGNOSTIC_CONTENT_TYPE = 'application/vnd.ores.internal-diagnostics+json';
...
const OUTCOMES = ['failed', 'dropped', 'suppressed', 'rejected', 'recovered'];
const PROVIDERS = ['aws_s3', 'gcp_cloud_storage', 'azure_blob'];
const GRANT_URLS = new WeakMap();
const GRANT_HEADERS = new WeakMap();
const CONSUMED_GRANTS = new WeakSet();
...
function ownDataValues(input, allowedKeys, requiredKeys = allowedKeys) {
...
function maybeCanonicalInternalDiagnosticRecord(record) {
    const value = ownDataValues(record, INTERNAL_DIAGNOSTIC_RECORD_KEYS);
    if (!value)
        return undefined;
    if (!(value.schema === INTERNAL_DIAGNOSTIC_SCHEMA &&
...
export function canonicalizeInternalDiagnosticRecord(record) {
    const canonical = maybeCanonicalInternalDiagnosticRecord(record);
    if (!canonical)
        throw new TypeError('invalid internal diagnostic record');
    return canonical;
}
 ...
 */
export function createOtelBridgeDiagnosticHook(reporter) {
...
export function createInternalDiagnosticBatch(records, now = () => new Date()) {
...
function validProviderHost(provider, hostname) {
...
function uniqueQueryValue(url, key) {
    const values = url.searchParams.getAll(key);
    return values.length === 1 && values[0] !== '' ? values[0] : undefined;
}
function validSignedHeaders(value) {
...
function signedExpirationMillis(provider, url) {
    ...
    const validAlgorithm = provider === 'aws_s3'
    ...
    if (!Number.isInteger(duration) || duration < 1 || duration > 900)
        return undefined;
    return signedAt + duration * 1000;
}
function validatedHeaders(provider, input) {
    ...
    if (contentType !== INTERNAL_DIAGNOSTIC_CONTENT_TYPE)
        return undefined;
    if (provider === 'azure_blob') {
...
export class InternalDiagnosticUploadGrant {
    ...
    expiresAt;
    maxBytes;
    constructor(provider, expiresAt, maxBytes) {
        this.provider = provider;
        this.expiresAt = expiresAt;
        this.maxBytes = maxBytes;
    }
    static parse(input, nowMillis = Date.now()) {
        ...
        if (!headers)
            throw new TypeError('internal diagnostic upload headers are invalid');
        const grant = new InternalDiagnosticUploadGrant(provider, expiresAt, maxBytes);
...
function canonicalBatch(batch) {
    ...
    let descriptors;
    try {
        descriptors = Object.getOwnPropertyDescriptors(value.records);
    }
    catch {
        return undefined;
    }
...
export async function uploadInternalDiagnosticBatch(grant, batch, options = {}) {
...
//# sourceMappingURL=internal-diagnostics.js.map