export const ORES_SUPABASE_WEBSOCKET_PROTOCOL = 'ores-otel/ws-ingest/v1';
const DEFAULTS = {
    batchSize: 50,
    maxQueueSize: 2_000,
...
function assertTicket(ticket, allowedHosts) {
 ...
 */
export class SupabaseWebSocketIngestTransport {
    name = 'supabase-websocket-ingest';
    options;
    resolved;
    queue = [];
    socket = null;
    ...
    constructor(options) {
        ...
        this.resolved = {
            ...
            acknowledgementTimeoutMillis: integer(options.acknowledgementTimeoutMillis, DEFAULTS.acknowledgementTimeoutMillis, 1),
            reconnectBaseMillis,
            reconnectMaxMillis: Math.max(reconnectBaseMillis, integer(options.reconnectMaxMillis, DEFAULTS.reconnectMaxMillis, 0)),
    ...
    async write(record) {
        ...
        if (this.options.awaitAcknowledgement === true || this.queue.length >= this.resolved.batchSize) {
            ...
            if (this.options.awaitAcknowledgement === true)
                ...
                void delivery.catch((error) => this.reportError(error));
        }
        else {
            this.scheduleFlush();
        }
    ...
    async drainWithReconnect() {
        ...
        while (this.inFlight || this.queue.length > 0) {
            ...
            try {
                ...
                if (attempts >= this.resolved.maxReconnectAttempts)
                    throw error;
                attempts += 1;
                this.reconnects += 1;
    ...
    scheduleFlush() {
        if (this.flushTimer || !this.accepting || this.queue.length === 0)
            return;
    ...
    async delayFor(attempt) {
        const exponent = Math.min(20, Math.max(0, attempt - 1));
        const base = Math.min(this.resolved.reconnectMaxMillis, this.resolved.reconnectBaseMillis * 2 ** exponent);
...
//# sourceMappingURL=supabase-websocket-ingest.js.map