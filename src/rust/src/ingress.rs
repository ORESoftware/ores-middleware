...
pub enum BufferAdmissionError {
    #[error("request buffering limits must all be positive")]
    InvalidLimits,
    #[error("request has too many headers")]
    HeaderCountExceeded,
    #[error("request headers exceed the byte budget")]
    HeaderBytesExceeded,
    #[error("request header name is invalid or oversized")]
    HeaderNameInvalid,
    #[error("request header value is oversized")]
    HeaderValueExceeded,
    #[error("request body exceeds the configured limit")]
    BodyExceeded,
    #[error("upload exceeds the configured limit")]
    UploadExceeded,
    #[error("decompressed request exceeds the configured limit")]
    DecompressedExceeded,
    #[error("request decompression expansion ratio exceeds the configured limit")]
    DecompressionRatioExceeded,
}

/// Performs checks available from the request head before a body, multipart
/// upload, or decompressor is allowed to allocate a large buffer.
...
impl DecompressionBudget {
    ...
    /// Records a decompressor chunk before retaining the expanded output.
    /// Callers must discard the chunk when this function rejects it.
    pub fn admit_chunk(
        &mut self,
        compressed_bytes: u64,
        decompressed_bytes: u64,
...
/// Projects any internal failure into the same public representation for every
/// HTTP host adapter. `internal_diagnostic` is accepted deliberately so tests
/// can prove it never crosses the public boundary.
...
pub enum ProxyAdmissionError {
    #[error("forwarded authority metadata came from an untrusted socket peer")]
    UntrustedForwardingPeer,
    #[error("forwarded host is invalid")]
    InvalidForwardedHost,
    #[error("forwarded scheme is invalid")]
    InvalidForwardedScheme,
    #[error("forwarded client address is invalid")]
...
pub enum BrowserAdmissionError {
    ...
    OriginDenied,
    #[error("cookie-authenticated mutation requires CSRF validation")]
    CsrfRequired,
    #[error("credentialed browser policy may not use the wildcard origin")]
    CredentialedWildcardOrigin,
    #[error("session cookie is missing required security attributes")]
...
fn unsafe_method(method: &str) -> bool {
    return matches!(method.to_ascii_uppercase().as_str(), "POST" | "PUT" | "PATCH" | "DELETE");
}
...
mod tests {
    ...
    fn public_errors_are_adapter_equivalent_and_never_include_diagnostics() {
        let expected = CERTIFIED_HTTP_INGRESS_ADAPTERS
            .iter()
            .map(|adapter| {
                let problem = public_problem(
                    *adapter,
                    500,
                    "internal_error",
                    Some("req-123"),
                ...
                encode_public_problem(&problem)
            })
            .collect::<Vec<_>>();
        assert!(expected.windows(2).all(|pair| pair[0] == pair[1]));
        let text = String::from_utf8(expected[0].clone()).expect("problem JSON");
    ...
    fn forwarded_metadata_is_admitted_only_from_configured_ingress() {
        ...
        let untrusted = policy.resolve(ForwardedRequest {
        ...
        assert_eq!(untrusted, Err(ProxyAdmissionError::UntrustedForwardingPeer));
