...
use std::{future::{pending, ready}, io, time::Duration};

use ores_middleware::{
    current_context, run_operation_boundary,
    ...
    OperationTransport, RequestContext,
};
...
fn context(slot: usize, message: bool) -> RequestContext {
    RequestContext {
        request_id: format!("tcp-{slot}-{}", if message { "message" } else { "connection" }),
...
fn descriptor(scope: OperationScope) -> OperationDescriptor {
    OperationDescriptor { transport: OperationTransport::Tcp, scope, name: "test.frame".into() }
}
...
async fn pair() -> (TcpStream, TcpStream) {
    ...
    let (client, accepted) = tokio::join!(TcpStream::connect(listener.local_addr().unwrap()), listener.accept());
    (client.unwrap(), accepted.unwrap().0)
}

async fn assert_closed(client: &mut TcpStream) {
    let mut byte = [0];
    let result = tokio::time::timeout(Duration::from_secs(3), client.read(&mut byte)).await.unwrap();
    match result {
        Ok(0) => (),
        Err(error) if matches!(error.kind(), io::ErrorKind::ConnectionReset | io::ErrorKind::BrokenPipe) => (),
...
async fn cancelled_connection_closes_owned_socket_without_polling_read() {
    ...
    let outcome = run_operation_boundary_with_timeout_and_cancellation(
        context(1, false), descriptor(OperationScope::Connection), Duration::from_secs(3), ready(()),
        async move { read_frame(&mut server).await },
    ).await;
    assert_eq!(outcome.failure().unwrap().kind, OperationFailureKind::Cancelled);
...
async fn partial_frame_deadline_closes_socket_and_restores_context() {
    ...
    let outcome = run_operation_boundary_with_timeout_and_cancellation(
        context(2, true), descriptor(OperationScope::Message), Duration::from_millis(25), pending(),
        async move { read_frame(&mut server).await },
    ).await;
...
async fn truncated_frame_disconnect_is_a_correlated_error() {
    ...
    let outcome = run_operation_boundary_with_timeout_and_cancellation(
        context(3, true), descriptor(OperationScope::Message), Duration::from_secs(3), pending(),
        async move { read_frame(&mut server).await },
    ).await;
    let failure = outcome.failure().unwrap();
...
async fn invalid_length_is_rejected_without_waiting_for_payload() {
    for length in [0, 65, u32::MAX] {
        ...
        let outcome = run_operation_boundary_with_timeout_and_cancellation(
            context(4, true), descriptor(OperationScope::Message), Duration::from_secs(3), pending(),
            async move { read_frame(&mut server).await },
        ).await;
...
async fn parallel_tcp_connections_keep_message_and_connection_scopes_separate() {
            ...
            for slot in 1..=24 {
                ...
                connections.spawn(async move {
                    ...
                    let outcome = run_operation_boundary(
                        outer.clone(), descriptor(OperationScope::Connection), async {
                            for _ in 0..2 {
                                let inner = context(slot, true);
                                let message = run_operation_boundary_with_timeout_and_cancellation(
                                    inner.clone(), descriptor(OperationScope::Message), Duration::from_secs(5), pending(),
                                    async {
    ...
    }).await.expect("loopback test must not hang");
}
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        let server_task = async {
            for expected in [OperationFailureKind::Panic, OperationFailureKind::Error] {
                ...
                let outcome = run_operation_boundary_with_timeout_and_cancellation(
                    context(5, true), descriptor(OperationScope::Message), Duration::from_secs(3), pending(),
                    async {
                        if body == b"panic" { panic!("synthetic message panic"); }
                        Err::<(), _>(io::Error::other("private message error"))
                    },
                ).await;
                assert_eq!(outcome.failure().unwrap().kind, expected);
                write_frame(&mut server, outcome.failure().unwrap().code.as_bytes()).await.unwrap();
                assert!(current_context().is_none());
                assert_eq!(current_log_context(), Default::default());
            }
            let body = read_frame(&mut server).await.unwrap();
            let result = run_operation_boundary(context(6, true), descriptor(OperationScope::Message), async {
                Ok::<_, io::Error>(body)
            }).await;
            let OperationOutcome::Completed(body) = result else { panic!("later message must work") };
            write_frame(&mut server, &body).await.unwrap();
        };
        let client_task = async {
            for (body, expected) in [("panic", "operation_panicked"), ("error", "operation_failed"), ("ok", "ok")] {
                write_frame(&mut client, body.as_bytes()).await.unwrap();
                assert_eq!(read_frame(&mut client).await.unwrap(), expected.as_bytes());
            }
        };
        tokio::join!(server_task, client_task);
    }).await.expect("message recovery test must not hang");
}
