...
use ores_middleware::{
    current_context, run_with_context, run_with_ores_log_context,
    run_operation_boundary_with_cancellation, run_operation_boundary_with_timeout,
...
async fn already_cancelled_never_polls_a_ready_handler() {
    for transport in [OperationTransport::Http, OperationTransport::Tcp, OperationTransport::WebSocket] {
        let dispatched = Arc::new(AtomicUsize::new(0));
        for slot in 0..128 {
            let count = dispatched.clone();
            let outcome = run_operation_boundary_with_cancellation(
                context(slot), descriptor(transport), ready(()),
                async move {
...
async fn zero_budget_never_polls_a_ready_handler() {
    for transport in [OperationTransport::Http, OperationTransport::Tcp, OperationTransport::WebSocket] {
        ...
        let outcome = run_operation_boundary_with_timeout(
            context(2), descriptor(transport), Duration::ZERO,
            async move {
...
async fn cancellation_drops_unpolled_handler_before_returning() {
    ...
    let outcome = run_operation_boundary_with_cancellation(
        context(3), descriptor(OperationTransport::Tcp), ready(()),
        DropProbe { polled: polled.clone(), dropped: dropped.clone() },
    ).await;
...
async fn zero_budget_drops_unpolled_handler_before_returning() {
    ...
    let dropped = Arc::new(AtomicBool::new(false));
    let outcome = run_operation_boundary_with_timeout(
        context(4), descriptor(OperationTransport::WebSocket), Duration::ZERO,
        DropProbe { polled: polled.clone(), dropped: dropped.clone() },
    ).await;
...
async fn pending_cancellation_allows_handler_and_preserves_context() {
    ...
    let outcome = run_operation_boundary_with_cancellation(
        expected.clone(), descriptor(OperationTransport::Tcp), pending(),
        async {
...
async fn cancelled_child_restores_outer_request_and_logger_context() {
    ...
    run_with_context(outer.clone(), run_with_ores_log_context(&log_source, async {
        let outer_log = current_log_context();
        let outcome = run_operation_boundary_with_cancellation(
            context(7), descriptor(OperationTransport::WebSocket), ready(()),
...
async fn concurrent_cancelled_operations_do_not_dispatch_or_leak_context() {
    let tasks = (10..74).map(|slot| async move {
        let outcome = run_operation_boundary_with_cancellation(
            context(slot), descriptor(OperationTransport::Tcp), ready(()),
            async { panic!("a cancelled operation must never dispatch"); #[allow(unreachable_code)] Ok::<(), Infallible>(()) },
        ).await;
        assert_failure(&outcome, OperationFailureKind::Cancelled, slot);
        assert!(current_context().is_none());
        assert_eq!(current_log_context(), Default::default());
