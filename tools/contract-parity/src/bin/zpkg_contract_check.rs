...
const EXPECTED_WORKSPACE_SCRIPTS: &[(&str, &str)] = &[
    ...
    (
        "persistence:check",
        "npm run persistence:catalog && npm run persistence:data-plane",
    ),
...
fn parse_assignments(section: &str, body: &str) -> Result<BTreeMap<String, String>, Finding> {
    ...
    for (index, line) in body.lines().enumerate() {
        ...
        if let Some((_key, raw, depth)) = pending.as_mut() {
            raw.push('\n');
            raw.push_str(clean);
...
fn validate_required_files(root: &Path, findings: &mut Vec<Finding>) {
    ...
    if let Some(matrix) = read_text(root, "scripts/orm_matrix_gate.py", findings) {
        for lane in [
            "typespec-diesel",
            "typespec-seaorm",
            ...
            if !matrix.contains(lane) {
                findings.push(finding(
                    FindingCode::OrmMatrix,
                    "scripts/orm_matrix_gate.py",
                    format!("four-way ORM matrix must retain lane {lane:?}"),
                ));
            }
        }
        for fragment in [
            ...
            if !matrix.contains(fragment) {
                findings.push(finding(
                    FindingCode::OrmMatrix,
                    "scripts/orm_matrix_gate.py",
                    format!("ORM matrix must root-anchor evidence paths: missing {fragment:?}"),
                ));
            }
        }
    }

    if let Some(compatibility) = read_text(root, "scripts/orm_catalog_gate.py", findings) {
        for fragment in [
            "from scripts.orm_matrix_gate import main as matrix_main",
            "return matrix_main()",
        ] {
            if !compatibility.contains(fragment) {
                findings.push(finding(
                    FindingCode::OrmCompatibility,
                    "scripts/orm_catalog_gate.py",
                    format!(
                        "legacy ORM entrypoint must delegate to the complete matrix: missing {fragment:?}"
                    ),
                ));
            }
        }
    ...
    if let Some(data_plane) = read_text(root, "scripts/orm_data_plane_gate.py", findings) {
        for lane in [
            "typespec-diesel",
            "typespec-seaorm",
            "json-schema-openapi-diesel",
            ...
            if !data_plane.contains(lane) {
                findings.push(finding(
                    FindingCode::OrmDataPlane,
                    "scripts/orm_data_plane_gate.py",
                    format!("row-level ORM matrix must retain lane {lane:?}"),
                ));
            }
        }
        for fragment in [
            ...
            if !data_plane.contains(fragment) {
                findings.push(finding(
                    FindingCode::OrmDataPlane,
                    "scripts/orm_data_plane_gate.py",
                    format!(
                        "row-level gate must execute real Diesel and SeaORM paths for both authorities: missing {fragment:?}"
                    ),
                ));
            }
        }
    ...
    if let Some(workflow) = read_text(
        ...
        for fragment in [
            ...
            if !workflow.contains(fragment) {
                findings.push(finding(
                    FindingCode::PersistenceWorkflow,
                    ".github/workflows/persistence-convergence.yml",
                    format!(
                        "persistence CI must retain catalog and row-level gates: missing {fragment:?}"
                    ),
                ));
            }
        }
...
mod tests {
    ...
    #[test]
    fn orm_matrix_without_root_anchoring_is_rejected() {
        let fixture = fixture();
        let path = fixture.path().join("scripts/orm_matrix_gate.py");
        let source = fs::read_to_string(&path).expect("read ORM matrix");
        fs::write(
            path,
            source.replacen("def resolve_cli_path(", "def untrusted_relative_path(", 1),
        )
        .expect("write drifted ORM matrix");
        assert!(has_code(&validate(fixture.path()), FindingCode::OrmMatrix));
    }

    #[test]
    fn legacy_orm_entrypoint_without_matrix_delegation_is_rejected() {
        let fixture = fixture();
        let path = fixture.path().join("scripts/orm_catalog_gate.py");
        let source = fs::read_to_string(&path).expect("read ORM compatibility shim");
        fs::write(path, source.replacen("return matrix_main()", "return 0", 1))
            .expect("write drifted ORM compatibility shim");
        assert!(has_code(
            &validate(fixture.path()),
            FindingCode::OrmCompatibility
        ));
    ...
    #[test]
    fn incomplete_row_level_orm_matrix_is_rejected() {
        let fixture = fixture();
        let path = fixture.path().join("scripts/orm_data_plane_gate.py");
        let source = fs::read_to_string(&path).expect("read ORM data-plane gate");
        fs::write(
            path,
            source.replace("json-schema-openapi-seaorm", "missing-seaorm-lane"),
        )
        .expect("write drifted ORM data-plane gate");
        assert!(has_code(
            &validate(fixture.path()),
            FindingCode::OrmDataPlane
        ));
    ...
    #[test]
    fn persistence_workflow_without_row_level_receipt_is_rejected() {
        let fixture = fixture();
        let path = fixture
            .path()
            .join(".github/workflows/persistence-convergence.yml");
        let source = fs::read_to_string(&path).expect("read persistence workflow");
        fs::write(
            path,
            source.replace(
                "target/orm-data-plane-gate/receipt.json",
                "target/legacy-only/receipt.json",
            ),
        )
        .expect("write drifted persistence workflow");
        assert!(has_code(
            &validate(fixture.path()),
            FindingCode::PersistenceWorkflow
        ));
