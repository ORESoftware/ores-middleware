...
import { checkRequestContract } from "./request-contract.js";
import { operationContextFromRequestContext, runOperationBoundary } from "./operation.js";
...
export function defaultConfig(serviceName) {
    return {
        contractVersion,
...
export function validateConfig(config) {
    ...
    if (config.contractVersion !== contractVersion)
        issue("/contractVersion", "unsupported_version", `expected ${contractVersion}`);
    if (!Number.isFinite(config.settings.timeoutMs) || config.settings.timeoutMs <= 0)
        issue("/settings/timeoutMs", "range", "timeout must be positive");
    if (!Number.isSafeInteger(config.settings.maxBodyBytes) || config.settings.maxBodyBytes <= 0)
        issue("/settings/maxBodyBytes", "range", "body limit must be a positive safe integer");
    if (config.settings.rateLimit.enabled && (config.settings.rateLimit.capacity <= 0 || config.settings.rateLimit.refillPerSecond <= 0))
        issue("/settings/rateLimit", "invalid_rate_limit", "enabled token bucket requires positive capacity and refill");
    if (config.settings.faultInjection.errorRate < 0 || config.settings.faultInjection.errorRate > 1 || config.settings.faultInjection.dropRate < 0 || config.settings.faultInjection.dropRate > 1)
        issue("/settings/faultInjection", "range", "fault rates must be within 0..=1");
    if (config.environment === "production" && config.settings.faultInjection.enabled)
        issue("/settings/faultInjection/enabled", "production_forbidden", "fault injection is forbidden in production");
    if (config.environment === "production" && config.settings.testAuthBypass.enabled)
        issue("/settings/testAuthBypass/enabled", "production_forbidden", "test auth bypass is forbidden in production");
    if (config.integrations.sharedAuth.failOpen)
        issue("/integrations/sharedAuth/failOpen", "auth_fail_open", "shared-auth must fail closed");
    if (config.settings.tls.mode === "trusted-proxy" && config.settings.tls.trustedProxyCidrs.length === 0)
        issue("/settings/tls/trustedProxyCidrs", "trusted_proxy_required", "trusted-proxy mode requires explicit CIDRs");
    for (const capability of config.requiredCapabilities)
        if (!capabilities.includes(capability))
            issue("/requiredCapabilities", "unknown_capability", capability);
    return issues;
}
...
class MemoryIdempotencyStore {
    ...
    async set(key, value) { this.#entries.set(key, value); }
}
export function createMiddleware(config, dependencies = {}) {
    const issues = validateConfig(config);
    if (issues.length > 0)
        throw new MiddlewareConfigError(issues);
    ...
    return async (request, next) => {
        ...
        const preAuthOutcome = await runOperationBoundary({ transport: "http", scope: "request", name: "middleware.pre_auth", signal: request.signal }, async () => {
            const contentLength = Number(request.headers.get("content-length") ?? "0");
            if (Number.isFinite(contentLength) && contentLength > config.settings.maxBodyBytes)
                return problem(413, "payload_too_large", "request body exceeds configured limit");
            const accepted = request.headers.get("accept");
            if (accepted && accepted !== "*/*" && !config.settings.contentRepresentations.some((representation) => accepted.includes(representation)))
                return problem(406, "not_acceptable", "no supported representation was requested");
            ...
            if (config.settings.tls.requireHttps && !effectiveHttps)
                return problem(426, "https_required", "HTTPS is required");
            if (config.settings.tls.strictForwardedHeaders && forwardedProto && !trustedProxy)
                return problem(400, "untrusted_forwarded_header", "forwarded transport headers came from an untrusted peer");
            if (dependencies.authorizeIp && !(await dependencies.authorizeIp(request, context)))
                return problem(403, "ip_policy_denied", "request source is not permitted");
            const contractFailure = await checkRequestContract(dependencies.requestContractValidator, request, url);
            if (contractFailure) {
            ...
            if (config.settings.rateLimit.enabled) {
                ...
                if (!(await rateLimiter.allow(rateKey, config.settings.rateLimit.capacity, config.settings.rateLimit.refillPerSecond)))
                    return problem(429, "rate_limited", "rate limit exceeded");
            }
            ...
            let auth = {};
            if (bypassRequested) {
                if (!canBypass || !dependencies.resolveTestIdentity)
                    return problem(403, "test_bypass_denied", "test identity bypass is unavailable");
            ...
            const authenticatedOutcome = await runOperationBoundary({ transport: "http", scope: "request", name: "middleware.request", signal: request.signal }, async () => {
                if (config.integrations.sharedAuth.mode !== "disabled" && !context.userId)
                    return problem(401, "authentication_required", "shared-auth did not establish a user");
                if (config.settings.faultInjection.enabled) {
                    if (config.settings.faultInjection.latencyMs > 0)
                        await delay(config.settings.faultInjection.latencyMs);
                    if (random() < config.settings.faultInjection.dropRate)
                        return problem(503, "fault_drop", "injected transport drop");
                    if (random() < config.settings.faultInjection.errorRate)
                        return problem(500, "fault_error", "injected middleware error");
                }
                ...
                if (dependencies.captureSchema)
                    await dependencies.captureSchema(request.clone(), response.clone());
                if (dependencies.syncObserver) {
                    try {
                        await dependencies.syncObserver(context, request.clone(), response.clone(), durationMs);
                    }
                    catch {
                        if (!config.integrations.optoSync.failOpen)
                            return problem(503, "sync_observer_failed", "opto-sync observation failed");
                    }
                }
...
export async function readJson(request, validator) {
    const contentType = request.headers.get("content-type") ?? "";
    if (!contentType.toLowerCase().includes("application/json"))
        throw new TypeError("expected application/json");
    const value = await request.json();
    if (validator && !validator(value))
        throw new TypeError("JSON body failed validation");
    return value;
}
export function sharedAuthHttpVerifier(config) {
    if (!config.introspectionUrl)
        throw new Error("shared-auth introspectionUrl is required for HTTP mode");
    return async (request) => {
        ...
        if (!response.ok)
            throw new Error("shared-auth introspection failed");
...
export function optoSyncHttpObserver(config) {
    if (!config.endpoint)
        throw new Error("opto-sync endpoint is required for HTTP mode");
    return async (context, request, response, durationMs) => {
        ...
        if (!result.ok)
            throw new Error(`opto-sync returned ${result.status}`);
    };
}
...
function withDeadline(timeoutMs, operation) {
...
function validToken(value) { return value && value.length <= 128 && /^[A-Za-z0-9._-]+$/.test(value) ? value : undefined; }
...
function operationFailureResponse(failure) {
...
function problem(status, code, detail) { return Response.json({ type: `urn:ores:middleware:${code}`, title: code, status, detail }, { status, headers: { "content-type": "application/problem+json" } }); }
function mergeVary(headers, ...tokens) {
...
function attachHeaders(config, context, response) {
    ...
    if (responseTraceparent)
        headers.set("traceparent", responseTraceparent);
    else
    ...
    if (config.settings.securityHeaders.enabled) {
        ...
        if (config.settings.securityHeaders.contentSecurityPolicy)
            headers.set("content-security-policy", config.settings.securityHeaders.contentSecurityPolicy);
    }
    return new Response(response.body, { status: response.status, statusText: response.statusText, headers });
}
async function attachEtag(request, response) {
    if (request.method !== "GET" || response.status !== 200 || response.headers.has("etag") || !response.body)
        return response;
...
function maybeCompress(config, request, response) {
    if (!config.settings.compression.enabled || !response.body || typeof CompressionStream === "undefined")
        return response;
    const length = Number(response.headers.get("content-length") ?? "0");
    if (!Number.isFinite(length) || length < config.settings.compression.minimumBytes || !request.headers.get("accept-encoding")?.includes("gzip"))
        return response;
    const headers = new Headers(response.headers);
