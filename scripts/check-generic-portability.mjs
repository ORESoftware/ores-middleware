...
for (const [language, filenames] of Object.entries(files)) {
  ...
  for (const filename of missing) fail(`${language}: missing ${path.relative(root, filename)}`);
  if (missing.length > 0) continue;

  const rawSources = filenames.map((filename) => fs.readFileSync(filename, "utf8"));
  const source = rawSources.join("\n");
  const shippedSource = rawSources.map((value) => productionSource(language, value)).join("\n");

  for (const symbol of required[language]) {
    if (!source.includes(symbol)) fail(`${language}: missing required generic concept ${JSON.stringify(symbol)}`);
  }

  const lower = shippedSource.toLowerCase();
  for (const provider of forbiddenProviderNames) {
    if (lower.includes(provider)) fail(`${language}: generic core mentions concrete provider ${provider}`);
  }
  for (const binding of forbiddenFrameworkBindings[language] ?? []) {
    if (lower.includes(binding)) fail(`${language}: generic core is coupled to framework binding ${JSON.stringify(binding)}`);
  }
  if (language === "typescript") {
    for (const binding of forbiddenTypeScriptRuntimeBindings) {
      if (lower.includes(binding)) fail(`typescript: generic core is coupled to runtime binding ${JSON.stringify(binding)}`);
    }
  }
}
