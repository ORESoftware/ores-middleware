#!/usr/bin/env node
/** Fail-closed TJSV gate for the portable runtime contract-drift event. */
...
export const DECLARATIONS = Object.freeze(['ContractDriftEvent']);
const REPORT_SCHEMA = 'ores.typespec-json-schema-validator.report/v1';
const CORPUS = 'contracts/tjsv-instances/runtime-drift';
const TYPESPEC = 'contracts/runtime-drift/typespec/main.tsp';
const JSON_SCHEMA = 'contracts/runtime-drift/json-schema/runtime-drift.schema.json';
const TYPESPEC_NAMESPACE = 'Ores.Middleware.RuntimeDrift';
...
/** Hash both peer authorities and every positive/negative fixture. */
  ...
  for (const declaration of DECLARATIONS) {
    ...
    for (const lane of ['invalid', 'valid']) {
      ...
      for (const filename of names) {
        ...
        corpusInstances += 1;
        assert(corpusInstances <= 64, 'too-many-fixtures');
      }
    }
  }
...
export function assertSuccessfulReceipt(report, corpusInstances) {
  ...
  for (const row of rows) {
    assert.equal(row.typespec, `${TYPESPEC_NAMESPACE}.${row.authored}`, 'typespec-identity-mismatch');
...
function freshEvidenceDirectory(root) {
  ...
  return mkdtempSync(join(root, 'target/tjsv/runtime-drift-'));
}

export function runRuntimeDriftGate(root = ROOT, execute = spawnSync) {
  ...
  const gate = {
    schema: 'ores.middleware.tjsv-runtime-drift-gate/v1',
    status: 'failed',
    scope: 'portable-runtime-drift-event-only',
  ...
  try {
    ...
    throw new Error('tjsv-runtime-drift-gate-failed', { cause: error });
  }
}
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(json(runRuntimeDriftGate()).trim());
  } catch {
    console.error(json({ status: 'failed', code: 'tjsv-runtime-drift-gate-failed' }).trim());
    process.exitCode = 1;
  }
}
