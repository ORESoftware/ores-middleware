...
const middleware = createMiddleware(config, {
  authVerifier: async request => ({
    userId: request.headers.get("x-test-user") ?? "local-test",
    tenantId: "test-only"
  }),
...
const handle = adapter(middleware, async (request, runtimeContext) => {
  calls++;
  await new Promise(resolve => setTimeout(resolve, 1));
  assert.equal(currentContext()?.userId, request.headers.get("x-test-user") ?? "local-test");
  if (runtimeContext?.params) assert.equal(await runtimeContext.params, "preserved");
...
await Promise.all(
  Array.from({ length: 32 }, (_, i) =>
    handle(request("http://127.0.0.1/echo", "ok", `user-${i}`))
  )
);
...
console.log(JSON.stringify({
  schema: "ores.middleware.native-fetch-smoke/v2",
  status: "passed",
  adapter: adapterName,
  runtime: globalThis.Bun?.version ?? globalThis.Deno?.version.deno ?? process.version,
  liveTransport,
