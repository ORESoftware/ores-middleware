...
const TJSV_REVISION = '733bc1b6b4b45433a38625fc4dd9ddb108b60519';
...
after(async () => {
  ...
  await writeFile(join(output, 'provenance.json'), `${JSON.stringify({
    ...
    sourceCommit,
    validatorRepository: 'ORESoftware/typespec-json-schema-validator',
    validatorRevision: TJSV_REVISION,
    inputSha256: afterDigests,
    sourceIntegrity: 'verified',
    scope: 'IdempotencyRecord and IdempotencyStatus only; not universal native-runtime certification',
  }, null, 2)}\n`, { flag: 'wx' });
});
...
async function check(name, { tsp = source, authored = schema, cases = corpus.cases } = {}) {
  ...
  for (const item of cases) {
    const location = join(
      instances,
      'IdempotencyRecord',
      item.expect === 'accept' ? 'valid' : 'invalid',
    );
...
function assertPass(report) {
  ...
  for (const key of [
    ...
    'typespecOutOfScopeDeclarations',
  ]) {
  ...
  assert.deepEqual(
    report.declarationMap.map(({ authored }) => authored).sort(),
