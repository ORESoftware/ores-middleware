...
15. CI validates the corpus and runs both framework-neutral and Axum Rust shutdown tests from the exact source revision.

The TypeScript and Go implementations consume the same semantics. Their native tests remain implementation-specific so they can exercise each runtime's real scheduling, timer, and HTTP behavior rather than sharing one implementation under multiple wrappers.
