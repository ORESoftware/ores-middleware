...
The root and publishable TypeScript export maps must remain semantically identical. `scripts/check-ts-package-exports.mjs` requires explicit `./bun`, `./deno`, and `./shutdown` entrypoints in addition to the runtime-neutral `./generic`, `./context`, and `./adapters` boundaries.
